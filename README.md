...
- **Custom JSON** — browse to a pipeline config file (same schema as `metabwatch --config`), or use **Create custom config** to make a new folder with JSON, an RP CoreMS TOML copy, a README, and (targeted) a blank `monitored_compounds.csv` to fill in
