...
- GUI **Write starter folder** helper: collect input/output folders, targeted vs untargeted, and RP thresholds, then write a simplified `metabwatch_config.json` plus copies of the RP CoreMS TOML (and QC CSV when targeted). The new JSON is selected as Custom JSON so Start can run immediately.
